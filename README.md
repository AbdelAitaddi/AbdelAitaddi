## Hi there 👋, I'm Abdel Ait Addi

Innovative Senior Frontend Engineer with over 10 years of experience in developing, building, and managing scalable and robust enterprise Applications. Proficient in modern JavaScript libraries and frameworks, including Angular ❤️, TypeScript, RxJS, NgRx, GraphQL, Jest, and Cypress.io.

## 🚀 My Frontend Masterpiece Project 🚀

#### 💥 Angular-achitecture-signals 💥
> Boilerplate project for getting started with Signal-based Component Architecture, Angular Deferrable Views and the Control Flow syntax. All components are fully Signal-driven. Additionally, the project features a custom reactive state management system built on Signals, ensuring efficient and predictable state handling.
> This project represents a migration from a Component-First architecture (standalone) to a Signal-based Component Architecture, utilizing Angular CLI 19.

#### ✨ Angular-achitecture-standalone ✨
> Boilerplate project for getting started with Component-First architecture(standalone), Smart and Dumb Components, reactive state management und Angular material.
> This project represents a migration from Angular-achitecture-modular to a Component-First architecture(standalone), utilizing Angular CLI 16.

#### 💫 Angular-achitecture-modular 💫
> Boilerplate project for getting started with Modular architecture, Component / Container architecture, RxJS State Management and Angular Material.
